...
import type { RateLimitData } from '@shared/hub-types'
...
export function RateLimitPanel({ data, onRefresh }: Props) {
  if (!data) {
    return (
  ...
  const { core, search, graphql, codeSearch, history } = data
  const corePct = Math.round((core.used / core.limit) * 100)
  const chartData = history.map((used, i) => ({ i, used }))

  return (
    <PanelShell title="API Rate Limits" icon={<Zap size={14} />} onRefresh={onRefresh}>
      {/* Core gauge + sparkline */}
      ...
      {chartData.length > 1 && (
        <div className="mb-4 h-12">
          <div className="text-[10px] text-white/30 mb-1">Core usage (last {chartData.length} polls)</div>
